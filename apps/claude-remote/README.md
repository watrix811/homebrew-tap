...
- macOS
- [Node.js](https://nodejs.org/) **18〜22（LTS 推奨）**
- [Claude Code](https://claude.com/claude-code)（`claude` コマンドが PATH にある）
- **Xcode Command Line Tools**（node-pty のビルドに必要）: `xcode-select --install`
- `tmux`（推奨）: `brew install tmux`

> ⚠️ **node-pty はソースビルドが必須**です。配布されている prebuilt バイナリには macOS 用の `spawn-helper` が含まれておらず、PTY 起動が `posix_spawnp failed` で失敗します。本プロジェクトの `.npmrc` で `build-from-source=true` を指定済みなので、Xcode CLT さえ入っていれば `npm install` が自動でソースビルドします。
>
> ⚠️ **Node の最新版（非LTS, 例: v25）は避けてください**。ネイティブモジュールのビルドが壊れがちです。`nvm install 22 && nvm use 22` で LTS を使うのが確実です。

## セットアップ

```bash
cd apps/claude-remote
npm install      # .npmrc により node-pty をソースビルド（Xcode CLT 必須）
...
### トラブルシュート: `posix_spawnp failed`

起動時に `[monitor] failed to start idle detection: posix_spawnp failed.` が出る、またはスマホ接続時に端末が開けない場合、node-pty の `spawn-helper` がビルドされていません。次で確認・修復します:

```bash
ls -l node_modules/node-pty/build/Release/spawn-helper   # 無ければ未ビルド
nvm use 22                                                # LTS を使う
rm -rf node_modules package-lock.json
npm install                                               # .npmrc によりソースビルド
ls -l node_modules/node-pty/build/Release/spawn-helper   # 今度は存在するはず
```

`spawn-helper` が生成されれば解決です。ビルドで `xcrun: error` 等が出たら `xcode-select --install` を先に実行してください。
